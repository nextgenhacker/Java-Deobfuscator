...
public class Deobfuscator {
	...
	public static ClassMapping deobfuscate(String annotatedJarFile,
	...
	public static void main(String[] args) {
		...
		if (args.length < 3) {
			System.out
					.println("Usage: "
							+ args[0]
							+ " <Input Unobfuscated Class Path> <Input Obfuscated Class Path> [Unobfuscated Files to Consider]\n"
							+ "\n"
		...
		for (int i = 0; i < unobfuscatedClassFiles.length; i++) {
			try {
				...
				for (int j = 0; j < obfuscatedClassFiles.length; j++) {
					...
					confidence[i][j] = ComputeConfidence(unobfuscatedClassNode,
							obfuscatedClassNode);
	 ...
	 * ComputeConfidence
	 * 
	 * @param unobfuscatedNode
	 *            The ClassNode representation of the current unobfuscated
	 *            source class that is being searched for.
	 * @param obfuscatedNode
	 *            The ClassNode representation of the current obfuscated source
	 *            class that is being matched against.
	 * 
	 * @return The confidence that the unobfuscated source code was transformed
	 *         into the current obfuscated source code. The closer to 0 the
	 *         value is, the higher the confidence.
	 * 
	 */
	public static int ComputeConfidence(ClassNode unobfuscatedNode,
			ClassNode obfuscatedNode) {

		//
		// Early rejection step
		//
		// It should be a somewhat safe assumption that ProGuard will not:
		// 1) Add additional fields to classes (provided classes are not being
		// folded together)
		// 2) Add additional methods to classes (provided classes are not being
		// folded together)
		...
		if (unobfuscatedNode.fields.size() < obfuscatedNode.fields.size()
				|| unobfuscatedNode.methods.size() < obfuscatedNode.methods
						.size()
				|| unobfuscatedNode.interfaces.size() != obfuscatedNode.interfaces
						.size()
				|| unobfuscatedNode.innerClasses.size() < obfuscatedNode.innerClasses
						.size()) {
		...
		for (int i = 0; i < unobfuscatedNode.methods.size(); i++) {
			...
			// Get the return type and argument types and frequencies of the
			// unobfuscated method
			HashMap<Type, Integer> UnobfuscatedMethodSignatureStatistics = GetMethodSignatureStatistics((MethodNode) unobfuscatedNode.methods
					.get(i));
			Type UnobfuscatedReturnType = Type
					.getReturnType(((MethodNode) unobfuscatedNode.methods
							.get(i)).desc);
			...
			// Iterate over all methods in the obfuscated class to try and find
			// matches
			for (int j = 0; j < obfuscatedNode.methods.size(); j++) {
				HashMap<Type, Integer> ObfuscatedMethodSignatureStatistics = GetMethodSignatureStatistics((MethodNode) unobfuscatedNode.methods
						.get(i));
				Type ObfuscatedReturnType = Type
						.getReturnType(((MethodNode) unobfuscatedNode.methods
								.get(i)).desc);

				// If the return types are different, then we know that this is
				// not a potential match
				if (ObfuscatedReturnType.getSort() != UnobfuscatedReturnType
						.getSort()) {
					continue;
				}

				// Compute a "method confidence" that the two methods being
				// compared are the same
				int MethodConfidence = 0;

				// The confidence is essentially the sum of the differences in
				// the number of
				// method arguments of each type
				for (Type ParameterType : UnobfuscatedMethodSignatureStatistics
						.keySet()) {
					if (ObfuscatedMethodSignatureStatistics
							.containsKey(ParameterType)) {
						MethodConfidence += Math
								.abs(ObfuscatedMethodSignatureStatistics
										.get(ParameterType)
										- UnobfuscatedMethodSignatureStatistics
												.get(ParameterType));
					} else {
						MethodConfidence += UnobfuscatedMethodSignatureStatistics
								.get(ParameterType);
					}
				}

				// Add this method-confidence pair to the list of candidates
				PotentialMethodMatches.add(new Pair<MethodNode, Integer>(
						(MethodNode) obfuscatedNode.methods.get(j),
						MethodConfidence));
			}

			// No suitable matches for this method were found, this class likely
			// does not match the other class
	...
	/**
	 * 
	 * This method takes in a MethodNode and returns a HashMap mapping the
	 * argument types and frequency
	 * 
	 * @param m
	 *            The method node to compute argument statistics from
	 * @return A HashMap mapping argument types to their frequency
	 */
	public static HashMap<Type, Integer> GetMethodSignatureStatistics(
			MethodNode m) {
		...
		for (Type argumentType : argumentTypes) {
			result.put(
					argumentType,
					(result.containsKey(argumentType) ? result
							.get(argumentType) + 1 : 1));
		}
		return result;
	}
}
